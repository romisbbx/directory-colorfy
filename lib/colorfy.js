...
(function(){
	...
	var Colorfy = function( filepath, extraColors, opts ){
		...
		this.indexFileName = opts.indexFileName;
		...
		this.separateFolder = opts.separateFolder;
	};
	...
	Colorfy.prototype.writeFiles = function( destFolder ){
		...
		if (this.separateFolder && _.size(this.colorFiles) > 0) {
			destFolder += '/' + this.ofnNoExt;
		}
	...
	Colorfy.prototype.convert = function(){
		...
		for( var name in self.colors ){
			if( self.colors.hasOwnProperty(name) ){
				...
				newFileContents = self.originalContents.replace( /fill=\"(?!none)([^\"]*)\"/gim, 'fill="' + color + '"' );
				newFileContents = newFileContents.replace( /stroke=\"(?!none)([^\"]*)\"/gim, 'stroke="' + color + '"' );
	...
	Colorfy.prototype._getColorConfig = function( str, extraColors ){
		...
		if( colors ){
			...
			colors.forEach( function( color, i ){
				var name = indexFileName ? i : color;

				if( extraColors[ color ] ){
					colorObj[ name ] = extraColors[ color ];
				} else if( isHex( color ) ){
					colorObj[ name ] = "#" + color;
